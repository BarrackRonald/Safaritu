...
// Multi-step booking form for travelers.
// Step 1 — Select departure + party size + review price
// Step 2 — Enter personal details
// Step 3 — Pay (Stripe card or Mpesa) — wired in Day 5
...
export default function BookingForm({ tour, operator, preselectedDepartureId }: Props) {
  ...
  // ── Payment handlers (wired in Day 5) ───────────────────────────────────────
  ...
  async function handleMpesaPayment() {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) {
        setSubmitError(data.error ?? "Mpesa request failed. Please try again.");
        return;
      }
      // Move to waiting screen — Mpesa STK push sent to phone
      setStep(3 as any);
    } catch {
      setSubmitError("An unexpected error occurred. Please try again.");
    } finally {
          ...
          <div key={label} className="flex items-center flex-1">
            ...
            {i < STEPS.length - 1 && (
              <div className={`flex-1 h-0.5 mx-2 mb-4 transition-colors ${
                i < step ? "bg-green-400" : "bg-stone-200"
              }`} />
            )}
          </div>
        ))}
        ...
        <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => { setDetails((d) => ({ ...d, firstName: e.target.value })); setErrors((e2) => ({ ...e2, firstName: undefined })); }}
                placeholder="Jane"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => { setDetails((d) => ({ ...d, lastName: e.target.value })); setErrors((e2) => ({ ...e2, lastName: undefined })); }}
                placeholder="Mwangi"
          ...
          <div>
            ...
            <input
              type="email"
              value={details.email}
              onChange={(e) => { setDetails((d) => ({ ...d, email: e.target.value })); setErrors((e2) => ({ ...e2, email: undefined })); }}
          ...
          <div>
            ...
            <input
              type="tel"
              value={details.phone}
              onChange={(e) => { setDetails((d) => ({ ...d, phone: e.target.value })); setErrors((e2) => ({ ...e2, phone: undefined })); }}
              placeholder="+254 7XX XXX XXX"
              className={`${INPUT} ${errors.phone ? "border-red-400" : ""}`}
            />
            {errors.phone && <p className="text-xs text-red-500 mt-1">{errors.phone}</p>}
            <p className="text-xs text-stone-400 mt-1">Used for Mpesa payment and trip updates</p>
          </div>
      ...
      {step === 2 && (
        <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 space-y-6">
          ...
          <div className="bg-amber-50 rounded-xl p-4 text-sm space-y-1.5">
            <p className="font-semibold text-stone-900">Booking summary</p>
            <p className="text-stone-600">{tour.title}</p>
            <p className="text-stone-600">
              {formatDateRange(departure.startDate, departure.endDate)} · {partySize} guest{partySize !== 1 ? "s" : ""}
            </p>
            <p className="text-stone-600">
          ...
          {submitting && (
            <div className="flex items-center justify-center gap-3 py-4 text-stone-500 text-sm">
          ...
          <p className="text-xs text-stone-400 text-center">
            ...
            <a href="/terms" className="text-amber-700 hover:underline">Terms of Service</a>.
            The remaining balance of {formatCurrency(balance, tour.currency)} is due before departure.
          </p>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-10 text-center space-y-5">
          ...
          <p className="text-stone-500 leading-relaxed max-w-sm mx-auto">
            ...
            <span className="font-bold text-stone-900">
              {formatCurrency(deposit, tour.currency)}
            </span>{" "}
            has been sent to <span className="font-bold">{details.phone}</span>.
          </p>
          <p className="text-stone-400 text-sm">
            Enter your Mpesa PIN to complete the payment. This page will update automatically once confirmed.
          </p>
